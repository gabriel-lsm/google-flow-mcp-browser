...
async def run_e2e():
    ...
    gen_raw = await flow_generate_media(gen_params)
    g = json.loads(gen_raw)
    ...
    if generation_started:
        ...
        print("  Aguardando geracao e download (timeout: 120s para imagem)...")

        dl_params = AwaitDownloadInput(timeout_seconds=120)
        dl_raw = await flow_await_download_media(dl_params)
        d = json.loads(dl_raw)
